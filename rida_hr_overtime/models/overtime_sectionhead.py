...
class OvertimeSectionHead(models.Model):
    ...
    def create(self, vals):
        for val in vals:
            # إنشاء التسلسل أولاً
            val['name'] = self.env['ir.sequence'].next_by_code('overtime.sectionhead') or ' '

            # إنشاء السجل
            res = super(OvertimeSectionHead, self).create(val)
            ...
            for emp in employees:
                lines.append((0, 0, {
                    'employee_id': emp.id,
                    'analytic_account_id': emp.analytic_account_id.id if emp.analytic_account_id else False,
                }))
