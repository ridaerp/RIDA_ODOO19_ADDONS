...
class HrOvertimeBatch(models.Model):
    ...
    def default_get(self, fields):
        res = super(HrOvertimeBatch, self).default_get(fields)
    ...
    state = fields.Selection(
        [('draft', 'Draft'), ('wlm', 'Waiting Line Manager'), ('hr_off', 'HR Officer Approve'),
    ...
    employees_line_ids = fields.One2many(comodel_name="overtime.batch.line", inverse_name="request_id",
                                         string="Employees", copy=1)
    total_overtime = fields.Float( string='Total Overtime', store=True)
    total_tax = fields.Float( string='Total Tax', store=True)
    total_net_overtime = fields.Float( string='Total Net', store=True)
    ...
    overtime_auth_id = fields.Many2one('overtime.auth')
    overtime_section_id = fields.Many2one('overtime.sectionhead')

    @api.model
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].get('hr.overtime.batch') or ' '
        res = super(HrOvertimeBatch, self).create(vals)
        return res
    ...
    def unlink(self):
        ...
        return super(HrOvertimeBatch, self).unlink()
        ...
        for rec in self:
            if self.env.user.has_group('base.group_system'):
                ...
                if not line_manager or line_manager != rec.env.user:
                    raise UserError("Sorry. Your are not authorized to approve this document!")
        self.total_overtime = sum(_rec.overtime for _rec in self.employees_line_ids)
        self.total_tax = sum(_rec.tax for _rec in self.employees_line_ids)
        self.total_net_overtime = sum(_rec.net_overtime for _rec in self.employees_line_ids)
...
class OvertimeBatchLine(models.Model):
    ...
    request_id = fields.Many2one("hr.overtime.batch")
    ...
    gross = fields.Integer("Gross", compute='check_employee_id', copy=True,group="")
    ...
    max_normal_hours = fields.Float(string="Normal H")
    max_holiday_hours = fields.Float(string="Holidays H")
    max_work_nat_hours = fields.Float(string="Work Nature H")
    amount_normal_hours = fields.Float(compute='_compute_hours')
    amount_holiday_hours = fields.Float(compute='_compute_hours')
    amount_work_nat_hours = fields.Float(compute='_compute_hours')
    hour_rate = fields.Float(compute='_compute_hours', string='Hour Rate')
    amount_overtime_hour = fields.Float(compute='_compute_hours', string='Total Overtime Hours')
    ...
    @api.constrains('normal_hours', 'holiday_hours', 'work_nat_hours')
    def _contraints_hours(self):
        for rec in self:
            if rec.request_id.overtime_auth_id:
                record = self.env['overtime.batch.line'].sudo().search(
                    [('request_id.overtime_auth_id', '=', rec.request_id.overtime_auth_id.id),
                     ('employee_id', '=', rec.employee_id.id)])
                total_normal_hours = sum(rec.normal_hours for rec in record)
                total_holiday_hours = sum(rec.holiday_hours for rec in record)
                total_work_nat_hours = sum(rec.work_nat_hours for rec in record)
                message = ""
                if total_normal_hours > self.max_normal_hours:
                    message += f"Your maximum [Normal Overtime Hours] for [{rec.employee_id.name}] is {self.max_normal_hours} \n"
                if total_holiday_hours > self.max_holiday_hours:
                    message += f"Your maximum [Holiday Overtime Hours for [{rec.employee_id.name}] is {self.max_holiday_hours} \n"
                if total_work_nat_hours > self.max_work_nat_hours:
                    message += f"Your maximum [Work Natural Overtime Hours] for [{rec.employee_id.name}] is {self.max_work_nat_hours} \n"
                if message:
    ...
    def _compute_hours(self):
        for rec in self:
            if rec.normal_hours or rec.holiday_hours or rec.work_nat_hours or rec.gross:
                rec.amount_normal_hours = 1.5 * rec.normal_hours
                rec.amount_holiday_hours = 2 * rec.holiday_hours
                rec.amount_work_nat_hours = 1.5 * rec.work_nat_hours
                rec.hour_rate = rec.gross / 240
                rec.amount_overtime_hour = rec.amount_normal_hours + rec.amount_holiday_hours + rec.amount_work_nat_hours
                rec.overtime = rec.hour_rate * rec.amount_overtime_hour
                rec.tax = round(rec.overtime * 0.05)
                rec.net_overtime = rec.overtime - rec.tax
            else:
                rec.amount_normal_hours = 0.0
                rec.amount_holiday_hours = 0.0
                rec.amount_work_nat_hours = 0.0
                rec.hour_rate = 0.0
                rec.amount_overtime_hour = 0.0
                rec.overtime = 0.0
                rec.tax = 0.0
    ...
    def unlink(self):
        ...
        return super(OvertimeBatchLine, self).unlink()
