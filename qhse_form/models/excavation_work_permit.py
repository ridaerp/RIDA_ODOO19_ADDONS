...
class ExcavationWorkPermit(models.Model):
    ...
    def action_view_procedure(self):
        ...
        return {
            ...
            'name': _('Permit To Work Procedure'),
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('excavation.work.permit') or '/'
        return super(ExcavationWorkPermit, self).create(vals_list)
    ...
    work_site = fields.Many2one('work.site',string='Exact Location / الموقع الدقيق',ondelete='set null', tracking=True)
        ...
        for rec in self:
            ...
            if rec.is_need_ptw:
                related_ptws = [rec.linked_cold_ptw, rec.linked_blasting_ptw, rec.linked_confined_ptw,
                                rec.linked_hot_ptw, rec.linked_highway_ptw, rec.linked_lifting_ptw,
                                rec.linked_loto_ptw]
    ...
    def action_dept_approve(self):
        for rec in self:
            ...
            for dept in depts_to_approve:
                already_approved = rec.dept_approval_ids.filtered(
                    lambda a: a.department_id == dept
                )
                if not already_approved:
                    self.env['work.dept.approval'].create({
                        'excavation_id': rec.id,
            ...
            approved_departments = rec.dept_approval_ids.mapped('department_id')
            if all(dept in approved_departments for dept in rec.department_ids):
