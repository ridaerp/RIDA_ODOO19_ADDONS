...
class IctDeviceRequest(models.Model):
    ...
    employee_id = fields.Many2one("res.users", string="Employee", track_visibility='onchange',
    ...
    def create(self, vals):
        if vals.get('name_seq', 'New') == 'New':
            vals['name_seq'] = self.env['ir.sequence'].next_by_code('device.request') or 'New'
...
class IctServices(models.Model):
    ...
    # def get_line_manager(self):
    #     if self.service_id:
    #         self.servicess22_id = self.service_id.servicess_id
    #    
    ...
    #     if self.service_id:
    #         self.service_type_id = self.service_id.service_type_id

            # , compute='get_line_manager'

            # , compute='get_service_type'