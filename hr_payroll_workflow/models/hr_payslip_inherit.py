...
class hr_payroll_workflow(models.Model):
    ...
    # def _depends_struct_id(self):
    #     for rec in self:
    #         print('>>>>>>>>>>.rec',rec)
    #         allowed_input_types = rec.struct_id.input_line_type_ids
    #
    #         # Get existing input lines
    #         print('>>>>>>>>>>.allowed_input_types',allowed_input_types)
    #         # Create or update input lines
    #         for input_type in allowed_input_types:
    #
    #                 input_line = self.env['hr.payslip.input'].create({
    ...
    def compute_mazaya(self):
        for record in self:
            ...
            basic_sal =((record.contract_id.payroll_wage/30)*record.payslip_day)* 61/100
    ...
    def caculate_workdays_take_home(self):
        for rec in self:
            rec.analytic_account_id=rec.contract_id.analytic_account_id
            lines=self.env['hr.payslip.line'].search([('slip_id','=',rec.id),('code','=','TH')])
            for line in lines:
                rec.take_home=line.amount
            if rec.contract_id.date_start<=rec.date_to and rec.contract_id.date_start>=rec.date_from:
                d1=datetime.strptime(str(rec.contract_id.date_start),"%Y-%m-%d").day
        ...
        if self.salary_currency.id != self.env.company.currency_id.id:
            ...
            'amount_currency': amount_currency,
                'analytic_distribution': {
                    str(line.salary_rule_id.analytic_account_id.id or line.slip_id.contract_id.analytic_account_id.id): 100} if (
                            line.salary_rule_id.analytic_account_id or line.slip_id.contract_id.analytic_account_id) else False,
           
            # 'analytic_account_id': line.slip_id.analytic_account_id.id or line.salary_rule_id.analytic_account_id.id or line.slip_id.contract_id.analytic_account_id.id,
        }
        else:
            ...
            if line.salary_rule_id.apper_on_journal:
                ...
                return {
                    ...
                    'debit': debit,
                    'credit': credit,
                    'analytic_distribution': {str(line.salary_rule_id.analytic_account_id.id or line.slip_id.contract_id.analytic_account_id.id): 100} if (line.salary_rule_id.analytic_account_id or
                        line.slip_id.contract_id.analytic_account_id) else False,
                ...
                return {
                    ...
                    'debit': debit,
                    'credit': credit,
                    'analytic_distribution': {str(line.salary_rule_id.analytic_account_id.id or line.slip_id.contract_id.analytic_account_id.id): 100}
                    if (line.salary_rule_id.analytic_account_id or line.slip_id.contract_id.analytic_account_id) else False,
...
class hr_payroll_workflow_run(models.Model):
    ...
    # def unlink(self):
    #     if any(self.filtered(lambda payslip_run: payslip_run.state not in ('draft','verify','close'))):
    #         self.write({'state': 'draft'})
    #     if any(self.mapped('slip_ids').filtered(lambda payslip: payslip.state not in ('draft','verify','cancel'))):
    #         self.write({'state': 'verify'})
            
    #     return super(hr_payroll_workflow_run, self).unlink()
...
class HrContract(models.Model):
    ...
    # scarcity_allowance=fields.Float("Scarcity Allowance")
    ...
    #         rec.payslip_day=30
    #         lines=self.env['hr.payslip.line'].search([('slip_id','=',rec.id),('code','=','TH')])
    #         # workdays=self.env['hr.payslip.worked_days'].search([('payslip_id','=',rec.id),('code','=','WORK100')])
    #         for line in lines:
    #             rec.take_home=line.amount
    #         # for line in workdays:
    #         #     rec.payslip_day=line.number_of_days
...
class HrPayslinputType(models.Model):
    ...
    sequence = fields.Integer(string='Sequence')  # Default sequence value