    ...
    def _onchange_employee_id(self):
        for rec in self:
            if rec.sudo().employee_id.contract_id:
                ...
                rec.company_id = rec.sudo().employee_id.sudo().contract_id.company_id
                rec.currency_id = rec.sudo().employee_id.sudo().contract_id.currency_id
            else:
                raise UserError(f'The {rec.sudo().employee_id.name} Dosn\'t Have Contract')

    @api.model
    def create(self, values):
        values['name'] = self.env['ir.sequence'].next_by_code('hr.loan.req.seq')
