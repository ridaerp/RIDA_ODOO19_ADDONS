...
class Bills_Workflow(models.Model):
    ...
    assigned_to_accountant = fields.Many2one('res.users', 'Assign To', track_visibility='onchange' , domain= lambda self: [("groups_id", "=", self.env.ref("account.group_account_user").id)] )
    ######################################comment by ekhlas#########3
