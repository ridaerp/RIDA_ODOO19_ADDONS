...
class MaterialRequest(models.Model):
    ...
    def create(self, vals):
        for val in vals:
            ...
            request = super(MaterialRequest, self).create(val)
    ...
    def make_purchase_quotation(self):
        if self.env.user.has_group('base.group_system'):
            ...
            if self.assigned_to_supply.id != self.env.user.id or not self.env.user.has_groups(
                    'material_request.group_buyers'):
...
class MaterialRequestLine(models.Model):
    ...
    product_qty = fields.Float(string='Quantity', track_visibility='onchange', digits=(16, 2))
