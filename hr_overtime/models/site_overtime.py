...
class SiteOvertime(models.Model):
    _name='hr.site.overtime'
    _description = "Site Overtime"
    ...
    def comp_gross_salary(self):
        if self.employee_id:
            if not self.env['hr.employee'].sudo().browse(self.employee_id.id) :
                self.gross_salary = 0.0
            else:
                self.gross_salary = self.sudo().employee_id.payroll_wage
        else:
            pass
    ...
    def comp_basic_salary(self):
        if self.employee_id:
            if not self.env['hr.employee'].sudo().browse(self.employee_id.id) :
                ...
                self.basic_salary = self.sudo().employee_id.basic
        else:
            pass
    ...
    def create(self, vals):
        result = super(SiteOvertime, self).create(vals)
    ...
    # Write Function
    def write(self, vals):
        res = super(SiteOvertime,self).write(vals)
    ...
    def button_bp_cancel(self):
        for rec in self:
            if rec.state == 'bp':
                rec.state = "lm"
    def button_coo_site_approve(self):
        for rec in self:
    ...
    def button_coo_site_cancel(self):
        for rec in self:
            if rec.state == 'coo':
                rec.state = "hr_officer"
    ...
    def unlink(self):
        for rec in self:
            ...
            return super(SiteOvertime, self).unlink()
