...
class FlowmeterReadingSheet(models.Model):
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('flowmeter.log.sheet.sequence') or 'New'
...
class AdrLogSheet(models.Model):
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('adr.log.sequence') or 'New'
...
class StrippingLogSheet(models.Model):
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('stripping.log.sheet.sequence') or 'New'
...
class StrippingHourlyLine(models.Model):
    ...
    # @api.depends('strrip_id.flowmeter_reading.reading_ids.daily_flow')
    # def _compute_flow_rate_from_reading(self):
    #     for rec in self:
    #         rec.flow_rate = rec.strrip_id.flowmeter_reading.daily_flow if rec.strrip_id.flowmeter_reading else 0.0
...
class CrusherLogSheet(models.Model):
    ...
    # def create(self, vals):
    #     if vals.get('name', 'New') == 'New':
    #         vals['name'] = self.env['ir.sequence'].next_by_code('crusher.log.sequence') or 'New'
    #     return super(CrusherLogSheet, self).create(vals)
    @api.model
    def create(self, vals):
        if vals.get('form_type') == 'crusher':

            vals['name'] = (self.env['ir.sequence'].next_by_code('crusher.log.sequence')
                            or '/')
        elif vals.get('form_type') == 'stacker':
            vals['name'] = self.env['ir.sequence'].next_by_code('stacker.log.sequence') or '/'
        return super(CrusherLogSheet, self).create(vals)
...
class BeltScaleLog(models.Model):
    ...
    def create(self, vals):
        if vals.get('form_type') == 'crusher':
            ...
            vals['name'] = (self.env['ir.sequence'].next_by_code('crusher.belt.scale.reading') or '/')
        elif vals.get('form_type') == 'stacker':
            vals['name'] = self.env['ir.sequence'].next_by_code('stacker.belt.scale.reading') or '/'
...
class PregnantListSample(models.Model):
    ...
    def create(self, vals):
        if vals.get('type') == 'ps':
            vals['name'] = (self.env['ir.sequence'].next_by_code('pregnant.result') or '/')
        elif vals.get('type') == 'bs':
            vals['name'] = (self.env['ir.sequence'].next_by_code('barren.result') or '/')
