...
class MrpWorkcenter(models.Model):
    ...
    # crucher_line = fields.Boolean(default=False)
...
class WorkcenterProductivityLoss(models.Model):
    ...
    #         if self.env.context.get('default_is_block',False):
    #             for loss in self.loss_id:
    #                 if loss.id == 7:
    #                     timer.write({'date_end': fields.Datetime.now()})
    #                 else:
    ...
    def create(self, vals):
        if not vals.get("sequence"):
            vals["sequence"] = self.env["ir.sequence"].next_by_code("productivity.loss.log")
