...
class Delegation(models.Model):
    ...
    hr_access = fields.Boolean("have hr access?", compute="check_access")
    ...
    def _update_related_fields(self):
        ...
        for rec in self:
            ...
            if rec.employee_id:
                ...
                if user_id:
                    # Update groups_id
                    groups = user_id.sudo().groups_id.ids
            ...
            if rec.employee_grant_id:
                ...
                if user_id and grant_user_id:
                    all_groups = user_id.sudo().groups_id.ids
                    grant_employee_groups = grant_user_id.sudo().groups_id.ids
    ...
    def onchange_employee_grant_id(self):
        for rec in self:
            if not rec.employee_grant_id or not rec.employee_id:
                return
            user_id = rec.employee_id.user_id
            all_groups = user_id.sudo().groups_id.sudo().ids
            grant_employee_groups = rec.sudo().employee_grant_id.user_id.sudo().groups_id.ids
