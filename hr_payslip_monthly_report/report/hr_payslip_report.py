...
class HrPayrollReportView(models.Model):
    ...
    def _select(self):
        """
            ...
            """
        select_str = """
            min(psl.id),ps.id,emp.id as name,dp.id as 
    ...
    def _from(self):
        """
            ...
            Returns:
                str: SQL FROM clause with the following table joins:
                    ...
                    - hr_salary_rule_category (rl) on rl.id = rlu.category_id
        ...
        from_str = """
            ...
            join hr_salary_rule_category rl on rl.id = rlu.category_id

            left join hr_version vr on vr.id = ps.version_id
            left join hr_department dp on dp.id = vr.department_id

            left join hr_job jb on jb.id = vr.job_id
        ...
        """
            ...
            Returns:
                str: SQL GROUP BY clause with the following fields:
                    ...
                    - rlu.id
            """
        group_by_str = """group by ps.id,emp.id,dp.id,jb.id,cmp.id,
        ps.date_from,ps.date_to,ps.state,
            psl.total,psl.name,rlu.category_id,rl.id,rlu.id"""
        return group_by_str
