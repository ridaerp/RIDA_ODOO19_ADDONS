...
class IssuanceRequest(models.Model):
    ...
    #     for rec in self:
    #         if rec.state=='done':
    #             delivered =self.env['stock.picking'].search([('origin','=',rec.name),('state','=','done')])
    #             rec.delivered=True
    ...
    def create(self, vals):
        ...
        request = super(IssuanceRequest, self).create(vals)
    ...
    def button_to_lm(self):
        for rec in self:
            ...
            for line in rec.line_ids:
                ...
                #     raise UserError(_("%sThere is no Available Quantity,please remove the product") % line.product_id.name)

                # if line.qty_requested> line.qty_available:
                #     raise UserError(_("%s the QTY Demanded Quantity is greater than the  Available Quantity ") %line.product_id.name)
...
class IssuanceRequestLine(models.Model):
    ...
    product_uom_id = fields.Many2one('uom.uom', 'Product Unit of Measure',
                                     track_visibility='onchange')
    qty_requested = fields.Float(string='Requested Quantity', track_visibility='onchange',
                                 digits=dp.get_precision('Product Unit of Measure'))

    qty_issued = fields.Float(string='Issued Quantity', track_visibility='onchange',
                              digits=dp.get_precision('Product Unit of Measure'))
