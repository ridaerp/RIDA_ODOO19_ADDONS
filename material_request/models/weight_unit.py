...
class ChemicalSamples(models.Model):
    ...
    requested_by = fields.Many2one('res.users', 'Sender Name',
                                   default=lambda self: self.get_requested_by(), store=True, readonly=True)
    ...
    receive_name = fields.Many2one('res.users', 'Receive Name',
...
class WeightLandedCostLine(models.Model):
    ...
    unit_price = fields.Float("Unit Price", compute="get_unit_price_landedcost")
    total = fields.Float("Total", compute="_compute_total", store=True)
    discount = fields.Float("Discount ", compute="get_unit_price_landedcost")
    ...
    def get_unit_price_landedcost(self):
...
class WeightOrderLine(models.Model):
    ...
    def get_unit_price(self):
        ...
        for rec in self:
            ...
            price_tailing_list = self.env['tailing.purchase.price.list'].search([])
            
            if  rec.weight_id.is_tailing==True:
                for line in price_tailing_list:
                    partner_id = line.partner_id
                    discount = line.discount
                    price = line.unit_price

                    if rec.product_id.type == 'consu':
                        if rec.weight_id.rock_vendor.id == partner_id.id :
                            rec.unit_price = price/rec.product_qty
                            rec.discount = 0.0
                    else:
                        rec.unit_price = area_list.x_studio_unit_price
                        rec.discount = discount
