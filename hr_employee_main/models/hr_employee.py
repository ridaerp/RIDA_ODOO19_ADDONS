...
class HrEmployee(models.Model):
    ...
    first_contract_date = fields.Date(compute='compute_first_contract_date')
    

    @api.depends('version_ids')
    def compute_first_contract_date(self):
        for rec in self:
            versions = rec.version_ids.sorted('id')
            rec.first_contract_date = versions[:1].contract_date_start if versions else False


    parent_id = fields.Many2one('hr.employee', 'Manager', compute="_compute_parent_id", store=True, readonly=False,domain="[]")


    birthday = fields.Date('Date of Birth', groups="hr.group_hr_user,base.group_user", tracking=True)
