...
# custody Workflow (IF user type: site ) from requester →line manager→ finance manager → site → accountant
    ...
    company_id = fields.Many2one('res.company', string='Company', required=True,readonly=True, default=lambda self: self.env['res.company']._company_default_get('account.custody'))
...
# internal audit buttons
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('account.custody') or 'New'
        result = super(AccountCustody, self).create(vals)
        return result
